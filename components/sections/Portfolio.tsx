...
import { PROJECTS, PORTFOLIO_FILTERS } from "@/constants";
            ...
            {filtered.map((project) => (
              ...
              >
                <Link href={`/portfolio/${project.id}`} className="block w-full h-full">
                  ...
                  {/* Hover overlay */}
                  <div className="absolute inset-0 bg-accent/95 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <span className="bg-white text-accent px-6 py-2 rounded-full font-medium inline-flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                      View Project →
                    </span>
                  </div>
                </Link>
